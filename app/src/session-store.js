(function exposeSessionStore(root, factory) {
  ...
  function createSessionStore(storage) {
    ...
    // Diferente de archive: some de vez com a conversa. Quem chama e'
    // responsavel por apagar tambem a memoria semantica dela no backend.
    function remove(id) {
      const sessions = readAll();
      const restantes = sessions.filter((session) => session.id !== id);
      if (restantes.length === sessions.length) return false;
      writeAll(restantes);
