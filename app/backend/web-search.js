...
// Sem provedor forçado via env, escolhe o melhor disponível pelas chaves
// configuradas: Tavily (feito pra consumo por LLM, sem lixo de HTML) > Brave
// (índice próprio, rápido) > DuckDuckGo scraping (sem chave, mas mais estável
// que o antigo default Bing RSS, que é um endpoint não-documentado).
// Lida sob demanda (não cacheado no load do módulo) pra respeitar mudanças
// de env em tempo de execução e pra ficar testável sem precisar reimportar.
function detectDefaultProvider() {
  const forced = process.env.JARVIS_WEB_SEARCH_PROVIDER;
  if (forced) return String(forced).toLowerCase();
  if (process.env.JARVIS_TAVILY_API_KEY) return 'tavily';
  if (process.env.JARVIS_BRAVE_SEARCH_API_KEY) return 'brave';
  return 'duckduckgo';
}
...
async function fetchText(url, headers = {}, { method = 'GET', body } = {}) {
  try {
    ...
    const args = ['--fail', '--silent', '--show-error', '--location', '--max-time', '15', '--request', method];
...
// Tavily é feito especificamente pra consumo por agentes/LLM: devolve
// `content` já limpo (sem HTML) em vez de link+snippet cru. 1000 créditos
// grátis/mês, sem cartão — https://tavily.com.
async function searchTavily(query, maxResults) {
  const apiKey = process.env.JARVIS_TAVILY_API_KEY;
  if (!apiKey) throw new Error('Defina JARVIS_TAVILY_API_KEY para usar o provedor Tavily.');
  let payload;
  try {
    const raw = await fetchText(
      'https://api.tavily.com/search',
      { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
      { method: 'POST', body: JSON.stringify({ query, max_results: maxResults, search_depth: 'basic' }) },
    );
    payload = JSON.parse(raw);
  } catch (error) {
    throw new Error(`Tavily: ${error.message}`);
  }
  return (payload.results || []).map((item) => ({
    title: String(item.title || '').slice(0, 300),
    url: normalizeUrl(item.url),
    snippet: String(item.content || '').slice(0, 900),
  })).filter((item) => item.url && item.title);
}
...
async function searchWeb({ query, maxResults = 5 } = {}) {
  ...
  const results = await (searchByProvider[provider] || searchDuckDuckGo)(normalizedQuery, limit);
  return {
...
module.exports = { detectDefaultProvider, parseBingRss, parseDuckDuckGo, searchWeb };
